                    ...
                    <div class="panel-body">
                        <div class="_buttons">
                            <?php if (has_permission('contabilidade102', '', 'create')) : ?>
                                ...
                                <a href="<?= admin_url('contabilidade102/lancamentos/lancamento'); ?>"
                                ...
                                <table class="table table-striped table-lancamentos">
                                    ...
                                    <tbody>
                                        <?php foreach ($lancamentos as $lancamento) : ?>
                                            <?php
                                                // total_debito já vem calculado pelo model
                                            ...
                                            <tr>
                                                ...
                                                <!-- ► CORREÇÃO: usando $base_currency para evitar warnings -->
                                                <td class="text-right">
                                                    <?= app_format_money($valor_lancamento, $base_currency); ?>
                                                </td>

                                                <td>
                                                    <span class="label"
                                                          style="background-color:<?= ($lancamento['status'] == 'rascunho' ? '#777' : '#84c529'); ?>;
                                                                 border:1px solid <?= ($lancamento['status'] == 'rascunho' ? '#777' : '#84c529'); ?>">
                                                        <?= _l('contabilidade102_status_' . $lancamento['status']) ?: htmlspecialchars($lancamento['status']); ?>
                                                    </span>
                                                </td>
                                                <td>
                                                    <a href="<?= admin_url('contabilidade102/lancamentos/lancamento/' . $lancamento['id']); ?>"
                                                       class="btn btn-default btn-icon" title="<?= _l('edit'); ?>">
                                                       <i class="fa fa-pencil"></i>
                                                    </a>
                                                    <?php if (has_permission('contabilidade102', '', 'delete')) : ?>
                                                        <a href="<?= admin_url('contabilidade102/lancamentos/delete_lancamento/' . $lancamento['id']); ?>"
