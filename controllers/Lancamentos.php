...
class Lancamentos extends AdminController
{
    ...
    public function lancamento($id = '')
    {
        ...
        if (!empty($id)) { 
            ...
            if (!$data['lancamento']) {
                set_alert('danger', _l('contabilidade_lancamento_nao_encontrado'));
                redirect(admin_url($this->module_name . '/lancamentos'));
            }
            $data['title'] = _l('contabilidade_editar_lancamento_titulo'); 
        } else { 
        ...
        $this->form_validation->set_rules('data_lancamento', _l('contabilidade_data_lancamento'), 'required');
        $this->form_validation->set_rules('descricao_historico', _l('contabilidade_historico_lancamento'), 'required|min_length[5]');
        ...
        if ($this->input->post()) {
            // ... (lógica do POST como você postou) ...
             if ($this->form_validation->run() !== false) {
                ...
                if (!$validacao_itens['status']) {
                    ...
                    if (empty($id)) { 
                        ...
                        if ($lancamento_id) {
                            ...
                            set_alert('danger', _l('contabilidade_erro_cadastrar_lancamento'));
                        }
                    } else { 
                        // Certifique-se que a instância do model aqui também usa a capitalização correta
                        $success = $this->lancamentos_model->update_lancamento_completo($id, $post_data); 
                        if ($success) {
                            set_alert('success', _l('contabilidade_lancamento_atualizado_sucesso'));
                        } else {
                            set_alert('danger', _l('contabilidade_erro_atualizar_lancamento'));
                        }
                    }
                    redirect(admin_url($this->module_name . '/lancamentos'));
                }
               ...
               set_alert('danger', _l('contabilidade_erro_validacao_formulario_geral'));
            }
    ...
    private function _validar_itens_lancamento($itens)
    {
        if (empty($itens) || !is_array($itens)) {
            return ['status' => false, 'mensagem' => _l('contabilidade_erro_nenhum_item_lancamento')];
            }
        ...
        foreach ($itens as $key => $item) {
            ...
            if (empty($item['plano_conta_id']) || !isset($item['valor']) || !isset($item['tipo_movimento']) || !in_array($item['tipo_movimento'], ['D', 'C'])) {
                return ['status' => false, 'mensagem' => _l('contabilidade_erro_item_incompleto', $item_count)];
            }
            ...
            if (!is_numeric($valor_numerico) || $valor_numerico < 0) {
                return ['status' => false, 'mensagem' => _l('contabilidade_erro_valor_item_invalido', $item_count)];
            }
        ...
        if (abs($total_debito) < 0.001 && abs($total_credito) < 0.001 && $item_count > 0) {
            return ['status' => false, 'mensagem' => _l('contabilidade_erro_lancamento_zerado')];
        }
        ...
        if (abs($total_debito - $total_credito) > 0.001) {
            ...
            return ['status' => false, 'mensagem' => _l('contabilidade_erro_debitos_creditos_nao_batem', [app_format_number($total_debito, true), app_format_number($total_credito, true)])];
        }
        ...
        //      return ['status' => false, 'mensagem' => _l('contabilidade_erro_minimo_dois_itens')];
        // }
    ...
    public function delete_lancamento($id)
    {
        ...
        if (!$lancamento) {
            ...
            if ($success) {
                set_alert('success', _l('contabilidade_lancamento_excluido_sucesso'));
            } else {
                set_alert('danger', _l('contabilidade_erro_excluir_lancamento'));
            }
        }
        redirect(admin_url($this->module_name . '/lancamentos'));
    }
